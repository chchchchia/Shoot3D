...
public class BulletGL {
		...
		public BulletParticle[] particles = new BulletParticle[MAX_PARTICLES];
		...
		public BulletGL(int x, int y, double power, double angle,double ticks, int heading){
			this.x=x;
			this.xo=x;
			this.y=y;
			this.yo=y;
...
//			initPtcls(this.x, this.y);
		...
		public void initPtcls(float x, float y){
			...
			System.out.println("initptcls");
			this.ptclsActive=true;
		      for (int i = 0; i < MAX_PARTICLES; i++) {
		         particles[i] = new BulletParticle(x,y,this.heading);
//		         particles[i].setX(x);
//	           particles[i].setY(y);
		      }
//		      System.out.println("initptcls-Done!");
	  }
	}
	...
	class BulletParticle {
	    ...
	    // Constructor
	    public BulletParticle(float x, float y, float z) {
	    	//TODO use z value as heading
	       boolean active = true;
	       this.x=x;
	       this.y=y;
	       this.z=0;
